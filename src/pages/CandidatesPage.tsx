...
import { useGetCandidatesQuery, usePatchCandidateMutation } from "src/api/candidatesApi";
...
export default function CandidatesPage() {
  const { data, isLoading, isError, error } = useGetCandidatesQuery({ page: 1, pageSize: 20 });
  ...
  const rows = useMemo(
    () =>
      (data?.items ?? []).map((c) => {
        ...
        const show = iv && (iv.status === "success" || iv.status === "declined") && iv.scheduledAt;
        const statusCode = iv?.status ?? "—";
        const statusLabel = HR_STATUS_OPTIONS.find((o) => o.value === statusCode)?.label ?? statusCode;
        return {
          ...c,
          scheduledAtText: show ? new Date(iv.scheduledAt).toLocaleDateString() : "—",
          statusCode,
          statusLabel,
          meetLink: c.meetLink ?? iv?.meetLink,
        };
      }),
    [data]
  );
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", height: "100vh", minHeight: 0 }}>
      <Typography variant="h5" mb={2}>Кандидаты</Typography>
      ...
      <Box sx={{ flex: 1, minHeight: 0, width: "100%" }}>
        <DataGrid
          ...
          processRowUpdate={async (newRow, oldRow) => {
            ...
            if (Object.keys(body).length > 0) await patchCandidate({ id: n._id, body }).unwrap();
            return newRow;
          }}
